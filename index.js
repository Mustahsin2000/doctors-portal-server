...
async function run() {
  ...
  try {
    ...
    app.get('/v2/appoinmentOptions', async (req, res) => {
      const date = req.query.date;
      const options = await appoinmentOpption.aggregate([
        {
          $lookup: {
            from: 'bookings',
            localField: 'name',
    ...
    */
    ...
    app.get('/appoinmentSpeciality', async (req, res) => {
      const query = {};
      const result = await appoinmentOpption.find(query).project({ name: 1 }).toArray();
      res.send(result);
    })
    ...
    // //temporary to update price field on appoinment opptions
    ...
    //     }
    //   }
    //   const result = await appoinmentOpption.updateMany(filter,updatedDoc,options);
    //   res.send(result);
